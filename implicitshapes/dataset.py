...
class CustomRandomAffine(MapTransform):
    ...
    def __init__(self, keys, rotate_range=None, shear_range=None, translate_range=None, scale_range=None,
                 mode='bilinear', prob=0.1, padding_mode='border', device=None, output_key='affine_mtx'):
        super().__init__(keys)
...
class AddMaskChannel():
    ...
    def __call__(self, data_dict):
        ...
        if self.affine_mtx_key:
            ...
            # b/c MONAI stores the inverse transform, we apply an inverse
...
class MetaDataset(torch.utils.data.Dataset):
    def __init__(self, json_list, do_scale, do_rotate, do_pca):
