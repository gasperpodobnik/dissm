...
# from implicitshapes.modified_mesh_to_sdf import sample_sdf_near_surface, mesh_to_voxels, get_surface_point_cloud, mesh_to_sdf
...
# import pymesh
from implicitshapes.itk_utils import get_affine_from_itk, origin_adjust_from_pad
from implicitshapes.mesh_utils import scale_mesh, rigid_align_meshes, get_scale
...
def convert_binary2mesh(path, out_path):
    ...
    try:
        vertices, triangles, _, _ = measure.marching_cubes(
            smoothed, 0, allow_degenerate=False, method="lewiner"
        )
    except:
        vertices, triangles, _, _ = measure.marching_cubes_lewiner(
            smoothed, 0, allow_degenerate=False
        )
...
def convert_all_masks(in_folder, out_folder):
    ...
    paths = sorted(glob.glob(query_path))

    if not os.path.exists(out_folder):
        os.makedirs(out_folder)

    # find all .nii.gz files and convert them to mesh .obj files
    for cur_path in tqdm(paths, total=len(paths), desc="convert_all_masks"):
        # print(cur_path)
...
def scale_all_meshes(in_folder, out_folder):
    query_path = os.path.join(in_folder, "*.obj")
    paths = sorted(glob.glob(query_path))
    ...
    for cur_path in tqdm(paths, total=len(paths), desc="scale_all_meshes"):
        # print(cur_path)
        cur_filename = os.path.basename(cur_path)
        out_file = os.path.join(out_folder, cur_filename)
...
def register_meshes(in_folder, out_folder, anchor_mesh_path=None, scale=False):
    """
    ...
    if anchor_mesh_path is None:
        anchor_mesh_path = paths[0]
    paths = [p for p in paths if p != anchor_mesh_path]
    ...
    if scale:
        anchor_mesh = scale_mesh(anchor_mesh)

    # for every other mesh
    for cur_path in tqdm(paths, total=len(paths), desc="register_meshes"):
        # print(cur_path)
        cur_filename = os.path.basename(cur_path)
        out_file = os.path.join(out_folder, cur_filename)
        moving_mesh = trimesh.load(cur_path)
        if scale:
            moving_mesh = scale_mesh(moving_mesh)
...
def align_large_meshes(
    in_folder, aligned_mesh_folder, out_folder, anchor_mesh_path=None, scale=False
):
    """
    ...
    for cur_path in tqdm(paths, total=len(paths), desc="align_large_meshes"):
        # print(cur_path)
        cur_filename = os.path.basename(cur_path)
        out_file = os.path.join(out_folder, cur_filename)
...
def sample_sdf(
    ...
    for cur_path in tqdm(paths, total=len(paths), desc="sample_sdf"):
        # print(cur_path)
        cur_filename = os.path.basename(cur_path)
        out_file = os.path.join(out_folder, cur_filename + ".npz")
        pool.apply_async(
            sample_one_sdf_file,
            args=(cur_path, out_file, number_of_points, uniform_proportion, jitter),
        )
...
def sample_one_sdf_file(
    cur_path, out_path, number_of_points=500000, uniform_proportion=0.5, jitter=0.0025
):
    """
    ...
    print(cur_path)
    mesh = trimesh.load(cur_path)
    points, sdf = modified_sample_sdf_near_surface(
        mesh,
        number_of_points=number_of_points,
        uniform_proportion=uniform_proportion,
...
def modified_sample_sdf_near_surface(
    mesh,
    number_of_points=500000,
...
def render_sampled_pts(mesh_file):
    ...
    points, sdf = modified_sample_sdf_near_surface(
        mesh, surface_point_method="sample", number_of_points=100000
    )
...
def create_sample_json(in_folder):
    ...
    json_list = []
    for cur_path in tqdm(paths, total=len(paths), desc="create_sample_json"):
        # print(cur_path)
        cur_filename = os.path.basename(cur_path)
